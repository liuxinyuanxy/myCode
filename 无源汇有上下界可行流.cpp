...
struct Edge
{
	...
	Edge() {}
	Edge(int _from, int _to, int _cap, int _flow, vector<Edge>::iterator _rev) : from(_from), to(_to), cap(_cap), flow(_flow), rev(_rev) {}
};
...
int cnt = 0;
int n, m, s, t;
il vi insert(int u, int v, int c)
{
	e[v].push_back(Edge(v, u, 0, 0, e[u].end()));
	e[u].push_back(Edge(u, v, 0, c, e[v].end() - 1));
	return e[u].end() - 1;
}
bool bfs()
{
